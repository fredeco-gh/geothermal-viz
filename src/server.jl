...
function start_server(; host::AbstractString="127.0.0.1", port::Int=8080,
    ...
    # Note: simulation execution (run/start/status polling, reservoir image
    # rendering) used to live here as HTTP routes. It now runs on jutul-agent's
    # Julia kernel instead (see examples/geothermal-viz-app/capability.py in the
    # jutul-agent-geothermal repo) — this server only resolves a well's starting
    # parameters; running Fimbul and inspecting results happens through the agent.
